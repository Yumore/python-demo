...
def parse_execl(filename):
    ...
    for execl_sheet in execle_sheets:
        for sheet_row in execl_sheet.values:
            ...
            if '·' in str(data_list) or '・' in str(data_list):
                str_a = []
                if '·' in str(data_list[0]):
                    str_a = str(data_list[0]).split('·')
                if '・' in str(data_list[0]):
                    str_a = str(data_list[0]).split('・')
                str_b = []
                # 翻译--[校对] 的骚操作对应的列
                real_suffix = data_list[1]
                if len(data_list) > 2 and pandas.isnull(data_list[2]) and len(str(data_list[2])) > 0:
                    real_suffix = data_list[2]
                ...
                if data_list in execl_rows:
                    continue
                execl_rows.append(data_list)
...
def read_xls(file_name):
    ...
    xls_workbooks = xlrd.open_workbook(file_name)
...
def trim_data(execl_rows):
    for data_list in execl_rows:
        # 移除 \n “ " ' 【】 []
        # for index, val in enumerate(real_data):
        for index in range(len(data_list)):
            if pandas.isnull(data_list[index]):
                data_list[index] = def_value
            if 'str' != typeof(data_list[index]):
                continue
            data_list[index] = data_list[index].strip()
            # 第一次移除逗号
            if data_list[index].endswith(',') or data_list[index].endswith('，'):
                data_list[index] = data_list[index][0: len(data_list[index]) - 1]
            if data_list[index].startswith('"') and data_list[index].endswith('"'):
                data_list[index] = data_list[index].replace('"', '')
            if data_list[index].startswith("'") and data_list[index].endswith("'"):
                data_list[index] = data_list[index].replace("'", '')
            # 奇奇怪怪的符号组合 TODO MLGJ
            if (data_list[index].startswith('“') or data_list[index].startswith('”')) and (data_list[index].endswith('”') or data_list[index].endswith('“')):
                data_list[index] = data_list[index][1: len(data_list[index]) - 1]
            # 奇奇怪怪的符号组合 TODO MLGJ
            if data_list[index].startswith('"') and (data_list[index].endswith('”') or data_list[index].endswith('“') or data_list[index].endswith('" ')):
                data_list[index] = data_list[index][1: len(data_list[index]) - 1]
            if data_list[index].startswith('"') and data_list[index].endswith(''):
                data_list[index] = data_list[index][1: len(data_list[index])]
            if data_list[index].startswith('“') and '” ' not in data_list[index]:
                data_list[index] = data_list[index][1: len(data_list[index])]
            # 再次移除逗号
            if data_list[index].endswith(',') or data_list[index].endswith('，'):
                data_list[index] = data_list[index][0: len(data_list[index]) - 1]
            if data_list[index].startswith('"') or data_list[index].startswith('“') or data_list[index].startswith('”'):
                print('parse_execl========>[%s]' % data_list[index])
                # print('parse_execl========>“    ”========>%s' % data_list[index])
        # print(data_list)
    return execl_rows
...
def merge_trans(dart_file, trans_data):
    ...
    for dart_line in dart_lines:
        if ':' in dart_line and not dart_line.startswith('//') and '.dart' not in dart_line:
...
def compare_content(root_path):
    lang_china = find_files(root_path, 'zh_hans', '.dart')
    for execl in find_files(root_path, '', '.%s' % execl_ext):
        ...
        # 处理dart文件
        for dart_file in find_files(root_path, prefix, '.dart'):
...
def filter_trans(filename, prefix):
    ...
    for line_tmp in dart_lines:
        ...
        if ('en_us' == prefix and 'zh' in lang) or (lang == 'zh' and language.string_convert(real_line)):
            ...
            un_trans_lines.append(real_value)
        # else:
        # print("filter_trans======>%s, %s" % (lang, line_tmp))
...
def compare_dart(dart_file, prefix, china_langs):
    ...
    if len(results):
        if len(china_keys) - len(dart_keys):
            ...
            # else:
            print("merge_list========", results)
        #     merge_list(dart_lines, china_lines, results, dart_file)
    else:
...
def find_project(path):
    for root, dirs, files in os.walk(path):
        for file in files:
            if '.idea' in file or '.dart_tool' in file or 'build' in file or '.gradle' in file or 'resource' in file or '.vscode' in file or '.fvm' in file or 'test' in file:
                continue
            if file.endswith('.dart') or file.endswith('.json'):
                fullname = os.path.join(root, file)
                yield fullname
...
if __name__ == '__main__':
    ...
    # print(all_data)
    compare_content('.')
    # find_unused('zh_hans.dart', "D:/Workspace/nr_p2_global")
    # compare_keys('./common/zh_hans.dart', './common/ko_kr.dart')
