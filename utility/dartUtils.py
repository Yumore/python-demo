...
from utility import configs, sqliteUtils, langUtils, fileUtils
...
def foreach_local(root_path):
    ...
    for prefix in configs.languages:
        ...
        sqliteUtils.create_table(sql_string)
        for dart_file in fileUtils.find_files(root_path, prefix, configs.dart_ext):
            merge_sentence(dart_file, prefix, table_name)
    # sqliteUtils.merge_table()
...
def merge_sentence(dart_file, prefix, table_name):
    ...
    for dart_line in dart_lines:
        if key_included(dart_line):
            ...
            # 匹配翻译内容是否有这个数据
            result_data = sqliteUtils.query_data(prefix, local_key, local_value)
            if len(result_data):
                # 内容替换, 判断是否有翻译或者校对
                fileUtils.format_logger("merge_sentence", [local_value, result_data, langUtils.judge_lang(local_value)])
                dart_line = dart_line.replace(local_value, "{0}".format(result_data))
            else:
                result_data = sqliteUtils.convert_wifi(local_value)
            # 将本地的内容更新到数据库
            if prefix == configs.def_prefix:
                # 中文的时候要将简体转换成繁体
                sqliteUtils.insert_local('tradition_local', local_key, zhconv.convert(result_data, configs.def_zhTW), generate_clazz(dart_file))
            sqliteUtils.insert_local(table_name, local_key, result_data, generate_clazz(dart_file))
    ...
    # 将最终的数据写入到源文件
    fileUtils.write_file(dart_file, result_lines)
    # 将繁体写入文件
    if len(hant_lines):
        hant_file = dart_file.replace(prefix, 'zh_hant')
        fileUtils.write_file(hant_file, hant_lines)
    ...
    for dart_file in fileUtils.find_files(root_path, prefix, configs.dart_ext):
        ...
        for dart_line in parse_dart(dart_file):
            if key_included(dart_line):
                ...
                lang = langUtils.judge_lang(real_value)
                if 'zh' in lang or (prefix == 'ja_jp' and langUtils.string_convert(real_value) and 'zh' == lang):
        ...
        if len(origin_lines):
            ...
            fileUtils.write_file(file_name, origin_lines)
...
def compare_dart(dart_file, prefix):
    fileUtils.format_logger('compare_dart', [dart_file, prefix])
    dart_lines = parse_dart(dart_file)
    dart_keys = []
    for dart_line in dart_lines:
        if key_included(dart_line):
    ...
    if os.path.exists(china_file):
        ...
        fileUtils.format_logger('compare_dart', diff_keys)
        if len(china_keys) - len(dart_keys):
            ...
            fileUtils.format_logger("compare_dart", "{0}:::{1} 和 {2} key的个数相同".format(prefix, dart_file, china_file))
    else:
        fileUtils.format_logger('compare_dart', '{0} 不存在'.format(china_file))
...
def merge_list(long_list, short_list, diff_keys, dart_file):
    ...
    if len(diff_lines):
        ...
        fileUtils.write_file(dart_file, short_list)
...
def compare_keys(china_file, dart_file):
    ...
    fileUtils.format_logger('compare_keys', results)
    final_lines = []
    if len(results):
        for dart_line in dart_lines:
            if key_included(dart_line):
                ...
                final_lines.append(dart_line)
        fileUtils.write_file('%s.bak' % dart_file, final_lines)
...
def parse_dart(filename):
    ...
    for line in lines:
        ...
        if not line_temp.strip().startswith('//') and (line_temp.endswith(':') or line_temp.endswith('Keys')):
            ...
            continue
        if len(key_pre):
            fileUtils.format_logger('parse_dart', [key_pre, line_temp])
            line = key_pre.rstrip('\n') + ' ' + line.strip(' ')
            key_pre = ''
            append_times = 0
    ...
    file.close()
    # 将文件格式化之后输出
    fileUtils.write_file(filename, line_list)
    return line_list
...
def parse_value(full_line):
    ...
    if real_line.count(': ') >= 2:
        ...
        # fileUtils.format_logger('parse_value', [index, real_line])
    else:
    ...
    if real_line.startswith('"') and real_line.endswith('"'):
        # fileUtils.format_logger('parse_value', real_line)
...
def generate_clazz(dart_file):
    clazz_name = fileUtils.filter_filename(dart_file)
    ...
    # fileUtils.format_logger('generate_clazz', [dart_file, real_name])
    return real_name
...
def convert_json(root_path):
    prefix = 'zh_hans'
    for json_file in fileUtils.find_files(root_path, prefix, configs.json_ext):
        fileUtils.format_logger('convert_json', json_file)
        ...
        fileUtils.write_file(result_file, result_lines)
